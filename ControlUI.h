...
private:
    ...
    static void ChooseCustomColor(HWND hwndParent, int target) {
        ...
        cc.rgbResult = (target == 1) ? state.activeBg : (target == 2) ? state.outlineColor : state.inactiveBg; 
...
public:
    static HWND Create(HINSTANCE hInstance) {
        ...
        return CreateWindowExW(0, L"ControlClass", L"Overlay", WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX, 100, 100, 260, 365, NULL, NULL, hInstance, NULL); 
    }
};